import json, threading, time, uuid, queue, socket, os, random, requests, traceback
from typing import Any
from bs_webctl._ws import WebSocketServer, WebSocket
import bottle
from bottle import request

# Extension TID config path — generated on first run, pairs Python server with Chrome extension
_EXT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'extension_src')
_CDP_CFG = os.path.join(_EXT_DIR, 'config.js')

def _ensure_tid():
    if not os.path.exists(_CDP_CFG):
        try:
            os.makedirs(os.path.dirname(_CDP_CFG), exist_ok=True)
            with open(_CDP_CFG, 'w', encoding='utf-8') as f:
                f.write(f"const TID = '__ljq_{hex(random.randint(0, 99999999))[2:8]}';")
        except Exception as e:
            print(f'[WARN] CDP config init failed: {e} — advanced web features will be unavailable.')
